...
func attack_collision(collision_normal):
    velocity += 1000 * collision_normal
...
#        ((velocity.angle_to(-direction) == 0 or velocity.length() < final_speed) and decel)):
...
#        if velocity.length() > stats.speed * sprint:
#            accelerate(delta, tangent, stats.friction, stats.speed * sprint, true)
#        else:
